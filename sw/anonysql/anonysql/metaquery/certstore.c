...
int LoadRecordOwnerCerts(ANONYSQL_SESSION *as,char *tablename,int *matchedIDs,int matchedRows,char *certs,int *numCert)
{
	...
	*numCert = 0;
	for( i = 0 ; i < matchedRows ; i++){
		...
		//データベースから署名を取得
		sprintf(sql , "SELECT cert FROM RecordOwnerCert WHERE servername = '%s' AND databasename = '%s' AND tablename = '%s' AND recordid = '%d'" , as->dataServerName,as->databaseName,tablename,matchedIDs[i]);
...
int DeleteRecordOwnerCerts(ANONYSQL_SESSION *as,char *tablename,int *affectedIDs ,int affectedRows)
{
	...
	for( i = 0 ; i < affectedRows ; i++){
		sprintf(sql , "DELETE FROM RecordOwnerCert WHERE servername = '%s' AND databasename = '%s' AND tablename = '%s' AND recordid = '%d'" , as->dataServerName,as->databaseName,tablename,affectedIDs[i]);
