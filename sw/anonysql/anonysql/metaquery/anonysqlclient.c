...
int PostRequest(ANONYSQL_SESSION *as, char *sql, int *response)
{
	...
	//SQL実行要求送信
	command = SQL_EXEC;
	if( MiniSSL_Put( &(as->dataConn) ,  (const unsigned char *)&command , sizeof(int)) != sizeof(int) ){
		printf("failed to send request(sqlexec).\n");
		goto FAIL;
	}
	//リクエスト証明書サイズ送信
	size = LN_now_byte(as->accessCert);
	if( MiniSSL_Put( &(as->dataConn) ,  (const unsigned char *)&size , sizeof(int)) != sizeof(int) ){
		goto FAIL;
	}
	//リクエスト署名本体送信
	memset(w , 0 , sizeof(w));
	LN_get_num_c(as->accessCert , size , w);
	if( MiniSSL_Put( &(as->dataConn) ,  (const unsigned char *)w , size) != size ){
	...
	//リクエスト本文送信
	if( MiniSSL_Put( &(as->dataConn) ,  (const unsigned char *)sql , size) != size ){
		printf("failed to send plaintext request(sqlexec).\n");
		goto FAIL;
	}
	if( MiniSSL_Flush(&(as->dataConn)) < 0 ){
		printf("failed to send request(sqlexec).\n");
		goto FAIL;
	}
	//レスポンスコード受信
...
int *AnonysqlExecUpdate(ANONYSQL_SESSION *as, char *sql, int *affectedRows, int *matchedRows)
{
	...
	*matchedRows = 0;
	...
	//レコード所有者証明書の数を送信
	MiniSSL_Put( &(as->dataConn) ,  (const unsigned char *)&numCert , sizeof(int));
	//レコード所有者証明書本体送信
	if( MiniSSL_Put( &(as->dataConn) ,  certs , certsize) != certsize ){
...
int AnonysqlBeginTrans(ANONYSQL_SESSION *as)
{
	...
	//レコード所有者証明書ストアもトランザクション開始
	if( BeginTransaction(&(as->certStore)) != TRUE ){
		printf("failed to begin transaction(local record owner cert manager).\n");
		return FALSE;
	}
	return TRUE;
}
int AnonysqlRollback(ANONYSQL_SESSION *as)
{
	...
	//レコード所有者証明書ストアもロールバック
	if( RollbackTransaction(&(as->certStore)) != TRUE ){
		printf("failed to rollback(local record owner cert store).\n");
		return FALSE;
	}
...
int AnonysqlCommit(ANONYSQL_SESSION *as)
{
	...
	//レコード所有者証明書ストアもコミット
	if( CommitTransaction(&(as->certStore)) != TRUE ){
		printf("failed to commit(local record owner cert store).\n");
		return FALSE;
	}
	return TRUE;
}